...
public class AmbusherSwapAI implements ShipSystemAIScript {
	
	private CombatEngineAPI engine;
    private ShipwideAIFlags flags;
    private ShipAPI ship;
    private ShipSystemAPI system;
    private boolean skipedOnce; //skips once so spatial swap has priority
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        if (tracker.intervalElapsed()) {
            if (ship.getFluxTracker().isOverloadedOrVenting() || !ship.isAlive() || system.isActive() || system.isCoolingDown() || ship.getFluxLevel() > 0.35f) {
                return;
            }
            if(!ship.isDrone()) {
