...
public class AmbusherSwapStats extends BaseShipSystemScript {
    ...
    private static final Color EMP_CORE_COLOR = new Color(200, 175, 100, 255);
    private static final Color EMP_FRINGE_COLOR = new Color(150, 75, 25, 200);
    private static final Color JITTER_WEAPON_COLOR = new Color(200, 150, 50, 255);
    private static final Color JITTER_UNDER_COLOR = new Color(150, 125, 50, 150);
    ...
    public String getInfoText(ShipSystemAPI system, ShipAPI ship) {
    	...
    	switch(system.getState()) {
        	...
        	case ACTIVE: return "WEAPONS BOOSTED";
        	case IDLE: break;
        	default: return null;
        }        
        if(target == null || ship == null) {
    ...
    public StatusData getStatusData(int index, State state, float effectLevel) {
        switch (state) {
            case ACTIVE: {
                if (index == 0) {
                    return new StatusData("Weapons boosted", false);
                }
                break;
            }
            default:
    ...
    public void unapply(MutableShipStatsAPI stats, String id) {
		...
		if(ship != null) {
			ship.setWeaponGlow(0, JITTER_WEAPON_COLOR, EnumSet.of(WeaponType.BALLISTIC, WeaponType.MISSILE, WeaponType.ENERGY));
		}		
    }
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {        
	    ...
	    if(state == State.ACTIVE){
			if(!teleported) {
				...
				if(lockedTarget != null) {
					...
					if(lockedTarget.isAlive()) {
			            ...
			            for (MissileAPI missile : CombatUtils.getMissilesWithinRange(lockedTarget.getLocation(), 450)) {
			                if (missile.getOwner() != lockedTarget.getOwner()) {
			                    ...
			                    while (point == null && max > 0) {
			                        point = MathUtils.getRandomPointInCircle(lockedTarget.getLocation(), shipRadius);
			                        if (!CollisionUtils.isPointWithinBounds(point, lockedTarget)) {
			                            point = null;
			                        }
			                        max--;
			                    }
			                    ...
			                    Global.getCombatEngine().spawnEmpArc(ship, point, lockedTarget, empTarget, DamageType.ENERGY,
			                            50, 200, 600f, null, 2f, EMP_FRINGE_COLOR, EMP_CORE_COLOR);			                    
			                }
			            }
			            
			            for (DamagingProjectileAPI proj : CombatUtils.getProjectilesWithinRange(ship.getLocation(), 300)) {
			...
			stats.getBallisticRoFMult().modifyMult(id, 1.5f);
			stats.getEnergyRoFMult().modifyMult(id, 1.5f);
			stats.getMissileRoFMult().modifyMult(id, 1.5f);
			stats.getFluxDissipation().modifyMult(id, 1.5f,"Weapons Boosted");
			ship.setWeaponGlow(1, JITTER_WEAPON_COLOR, EnumSet.of(WeaponType.BALLISTIC, WeaponType.MISSILE, WeaponType.ENERGY));
